...
public class TestEnemy
{
	...
	{
		Command command = new Command(getNewCommandNo(), enemyField,
				"INIT_FIELD");
		return command;
	}
	...
	{
		Ship ship = new Ship(new ShipPosition(new Point(4, 5), "vertical"),
				ShipType.AIRCARRIER, 1);
		enemyField.setShipOnField(ship);
		ship = new Ship(new ShipPosition(new Point(2, 3), "vertical"),
				ShipType.DESTROYER, 2);
		enemyField.setShipOnField(ship);
		ship = new Ship(new ShipPosition(new Point(6, 7), "vertical"),
				ShipType.YELLOW_SUBMARINE, 3);
		enemyField.setShipOnField(ship);
	}
	...
	private void wait(int ms)
	{
		...
		{
			Logging.writeErrorMessage("Test Enemy -> " + e.toString());
		}
	}
}
